...
ip_addr = "192.168.89.140" # M720 (Smart SFP) IP address

profile = "profile0" # configuration profile <profile0 | profile1>

# If you use several services, 
# don't forget to specify their number in the y1564_params dictionary in the "service_count" parameter.
...
def connection(ip_addr):
    ...
    connection = requests.post(f"http://{ip_addr}/api", json=
    {
        "jsonrpc": "2.0",
        "id": 1,
        "method": "call",
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Connect to M720 using REST API. Configure, start and stop y1564 test on M720.')
    parser.add_argument('action', help='Use start/stop/show commands')
    args = parser.parse_args()

    if args.action == "start":
        print(f"Connect to {ip_addr}")
        connection_id = connection(ip_addr)
        print("Configure service 0 and y1564 test")
        y1564_config = configure_y1564(ip_addr, connection_id, y1564_params)
        service0_config = configure_y1564_service(ip_addr, connection_id, service0_params)
        print("Start y1564 test")
        start_y1564(ip_addr, connection_id)
        result_common_y1564 = show_y1564_results(ip_addr, connection_id)
        print(result_common_y1564)
    elif args.action == "stop":
        print(f"Connect to {ip_addr}")
        connection_id = connection(ip_addr)
        print("Stop y1564 test")
        stop_y1564(ip_addr, connection_id)
        result_common_y1564 = show_y1564_results(ip_addr, connection_id)
        print(result_common_y1564)
        ...
        result_common_y1564 = show_y1564_results(ip_addr, connection_id)
        result_cir_service0 = show_service_tests_results(ip_addr, connection_id, service=0, test="cir", 
                                                        part=y1564_params['step_count'])
        result_eir_service0 = show_service_tests_results(ip_addr, connection_id, service=0, test="eir")
        result_tp_service0 = show_service_tests_results(ip_addr, connection_id, service=0, test="tp")
        result_perf_service0 = show_service_tests_results(ip_addr, connection_id, service=0, test="perf")
        print(
